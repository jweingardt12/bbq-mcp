...
import { GetCookingGuidanceSchema, AnalyzeTemperatureSchema, GetTargetTemperatureSchema, ListProteinsSchema, EstimateCookTimeSchema, DetectStallSchema, GetCookingTipsSchema, CalculateRestTimeSchema, SimulateDeviceReadingSchema, ConvertTemperatureSchema, } from "./schemas/index.js";
import { AuthenticateSchema, GetDevicesSchema, GetLiveReadingsSchema, AnalyzeLiveTemperatureSchema, CheckAuthStatusSchema, } from "./schemas/auth.js";
import { getThermoWorksClient, resetThermoWorksClient, } from "./services/thermoworks.js";
import { getProteinProfile, getTargetTemperature, estimateCookTime, calculateStartTime, analyzeTemperature, detectStall, calculateRestTime, getCookingTips, convertTemperature, getRecommendedCookMethod, } from "./services/cooking.js";
import { formatCookingGuidanceMarkdown, formatTemperatureAnalysisMarkdown, formatProteinListMarkdown, formatStallDetectionMarkdown, formatRestTimeMarkdown, formatDeviceReadingMarkdown, formatTipsMarkdown, formatTargetTempMarkdown, } from "./services/formatting.js";
 ...
 */
server.registerTool("bbq_get_cooking_guidance", {
...
Examples:
  ...
  - "I want to serve pulled pork at 6pm" -> protein_type='pork_butt', serving_time='2024-12-25T18:00:00'`,
    ...
    try {
        ...
        const { targetTemp, pullTemp, doneness } = getTargetTemperature(params.protein_type, params.target_doneness);
        const estimate = estimateCookTime(params.protein_type, params.weight_pounds, cookMethod);
        let startTimeInfo;
        if (params.serving_time) {
            const servingDate = new Date(params.serving_time);
            startTimeInfo = calculateStartTime(params.protein_type, params.weight_pounds, cookMethod, servingDate);
        }
        ...
        const markdown = formatCookingGuidanceMarkdown(profile, params.weight_pounds, targetTemp, pullTemp, doneness, cookMethod, estimate, startTimeInfo);
        return {
 ...
 */
server.registerTool("bbq_analyze_temperature", {
...
Examples:
  ...
  - "Temperature hasn't moved in 2 hours" -> Include previous_readings for stall detection`,
    ...
    try {
        ...
        const analysis = analyzeTemperature(params.current_temp, params.target_temp, params.protein_type, params.cook_method, cookStart, previousReadings);
        if (params.response_format === "json") {
            const output = {
 ...
 */
server.registerTool("bbq_get_target_temperature", {
...
Examples:
  ...
  - "When is chicken done?" -> protein_type='chicken_whole'`,
    ...
    try {
        const profile = getProteinProfile(params.protein_type);
        const { targetTemp, pullTemp, doneness } = getTargetTemperature(params.protein_type, params.doneness);
        if (params.response_format === "json") {
            const output = {
                ...
                allDonenessOptions: Object.entries(profile.donenessTemps).map(([level, temp]) => ({
                    level,
                    displayName: DONENESS_INFO[level]?.displayName || level,
                    temperature: temp,
                })),
            };
            return {
 ...
 */
server.registerTool("bbq_list_proteins", {
 ...
 */
server.registerTool("bbq_estimate_cook_time", {
...
Examples:
  ...
  - "Time for hot and fast brisket" -> protein_type='beef_brisket', cook_method='smoke_hot_fast'`,
    ...
    try {
        const profile = getProteinProfile(params.protein_type);
        const estimate = estimateCookTime(params.protein_type, params.weight_pounds, params.cook_method, params.smoker_temp);
        if (params.response_format === "json") {
            const output = {
                protein: {
                    type: params.protein_type,
 ...
 */
server.registerTool("bbq_detect_stall", {
 ...
 */
server.registerTool("bbq_get_cooking_tips", {
 ...
 */
server.registerTool("bbq_calculate_rest_time", {
...
Examples:
  ...
  - "Pulled steak at 125°F for medium-rare" -> protein_type='beef_ribeye', current_temp=125, target_final_temp=130`,
    ...
    try {
        const result = calculateRestTime(params.protein_type, params.current_temp, params.target_final_temp);
        if (params.response_format === "json") {
            const output = {
                proteinType: params.protein_type,
                currentTemp: params.current_temp,
 ...
 */
server.registerTool("bbq_analyze_device_reading", {
...
Examples:
  ...
  - "Smoke shows 180°F on the meat probe"`,
    ...
    try {
        ...
        const meatProbe = params.probe_readings.find((p) => !p.probe_id.toLowerCase().includes("ambient") && !p.name?.toLowerCase().includes("ambient"));
        let analysis;
        if (meatProbe && params.protein_type && params.target_temp) {
            analysis = analyzeTemperature(meatProbe.temperature, params.target_temp, params.protein_type);
        }
        if (params.response_format === "json") {
            const output = {
        ...
        const markdown = formatDeviceReadingMarkdown(params.device_type, params.probe_readings, analysis);
        return {
            content: [{ type: "text", text: markdown }],
        };
    }
 ...
 */
server.registerTool("bbq_convert_temperature", {
 ...
 */
server.registerTool("thermoworks_authenticate", {
 ...
 */
server.registerTool("thermoworks_check_auth", {
 ...
 */
server.registerTool("thermoworks_get_devices", {
 ...
 */
server.registerTool("thermoworks_get_live_readings", {
    ...
    try {
        ...
        for (const reading of readings) {
            ...
            for (const [probeId, probe] of Object.entries(reading.probes)) {
                const alarmStr = probe.alarm_high || probe.alarm_low
 ...
 */
server.registerTool("thermoworks_analyze_live", {
...
Examples:
  ...
  - "Check the turkey on probe 2" -> protein_type='turkey_whole', probe_id='2'`,
    ...
    try {
        ...
        const proteinType = params.protein_type;
        ...
        const analysis = analyzeTemperature(probeData.temp, target, proteinType, undefined, // No cook method from device
